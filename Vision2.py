...
class Vision_2:
    ...
    def CamaraSettings(self,camara) :
        ...
        camara.set(cv2.CAP_PROP_FRAME_WIDTH, 840)#840
    ...
    def main(self,BroccoliAcceptedSizes,rango) :
       ...
       # Archivo = open(f'Documentos\corte_{cont}.txt', "w")
        ...
        #Archivo.write('*********************************************************************************\n')
        Videoresult = cv2.VideoWriter(r'video2.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (int(w), int(h)))
        if camara.isOpened() :
            ...
            while self.ciclo:
                ...
                for i in infoPandas:
                    if i =="person" :
                        ...
                        cv2.putText(frame, "Peligro un Joto", (0, h2+25), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0,0,0), 2)
